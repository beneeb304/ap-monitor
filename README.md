...
- **AP health metrics (Health tab)**: uptime, load, memory, overlay/flash usage, temperature, per-band radio noise floor, per-band **transmit power**, **clock skew**, and channel utilization (opt-in, see caveat below) per AP, with history charts and HA sensors — see [Interpreting health metrics](#interpreting-health-metrics)
...
- **Channel-drift alarm**: a `channel_changed` event fires when an AP's broadcast channel differs from the previous health sample — on a network with deliberately pinned channels, any change means something reverted (factory reset, config rollback, DFS radar fallback)
- **Clock-skew alarm**: each AP's wall clock is compared against the monitor's every health sample; a `clock_skew` event fires (once per episode) when it drifts >60s — a stuck clock usually means NTP/DNS is silently broken on the AP
- **MQTT event topics**: new-device, unknown-device, roam, AP up/down, reboot, flapping, channel-overlap, channel-changed, and clock-skew events on `ap_monitor/events/<kind>` for HA automations (randomized-MAC joins are segregated to `new_random` to keep alerts quiet)
...
- **History reset**: a guarded "Reset history…" button (and API) clears all events and history while keeping device names — for when the network has been deliberately re-tuned and old data no longer describes it
- **Self-monitoring**: an unauthenticated `/health` liveness endpoint for the HA add-on watchdog, a per-minute heartbeat log line (threads / memory / file descriptors by type), and a `faulthandler` watchdog that dumps all thread stacks to the log if the poll loop ever stalls
...
- **Noise floor (dBm)** — the radio's background interference level; more negative is better (−100 is quiet, −85 is noisy). A *rising* noise floor points at a new non-wifi interference source (microwave, baby monitor, USB-3 gear near antennas). On some MediaTek/mt76 devices (confirmed on a GL.iNet Flint 2 via direct `ubus call iwinfo info`), the driver reports the *same* noise value for both the 2.4GHz and 5GHz radios rather than measuring each independently — if your 2.4/5 GHz noise readings are always identical, that's this hardware limitation, not a bug in the poller.
- **TX power (dBm)** — each radio's current transmit power, straight from the driver. It should be boring: a value that never changes. A silent drop (firmware update, regulatory-domain change, thermal throttling on some SoCs) shrinks your coverage with no other symptom — if yard-edge devices got flaky and this number fell, you've found why. Note that many radios cap below the regulatory maximum in their board calibration data: setting a higher `txpower` in OpenWrt than the hardware allows just clamps to the real ceiling.
- **Clock skew (s)** — the AP's own wall clock minus the monitor's, sampled over the same SSH session. Anything within a few seconds is healthy; the tile turns red and a `clock_skew` event fires past 60s. A drifting or stuck clock almost always means NTP can't sync — and since NTP usually fails because DNS is broken on the AP, this tile is really a DNS/NTP canary (found in production: two APs ran nine days behind because their `lan` interface had no DNS server configured; nothing else surfaced it). A wrong clock corrupts the AP's own log timestamps and can break TLS.
...
- **Channel overlap** — computed from each AP's own broadcast channel (captured over the existing SSH session, no extra polling), so it's always on. 2.4 GHz channels are 5 MHz apart but ~22 MHz wide: only 1/6/11 (5 apart) are truly non-overlapping, so two APs within 4 channel numbers of each other are contending for the same airspace — a classic cause of flapping and slow wifi. Fires a `channel_overlap` event (dashboard badge, `ap_monitor/events/channel_overlap` in MQTT) when it starts and `channel_clear` when it's fixed, not a repeat every poll. Not applicable to 5/6 GHz, which have much wider regulator-enforced channel spacing (note: on 5 GHz at 40/80 MHz widths, two *different* channel numbers can still occupy the same block — e.g. ch36 and ch48 both live in the 36–48 80 MHz block — which this by-number check doesn't model).
- **Channel changed** — a `channel_changed` event fires whenever a radio's broadcast channel differs from the previous health sample. If you run auto-channel, occasional changes are normal; if you've pinned your channels deliberately, *any* change is alarm-worthy — it means a factory reset, a config rollback, or a DFS radar fallback quietly undid your plan.
...
- `GET /health` — liveness probe: returns a bare `ok`, **exempt from Basic Auth** (it exposes nothing). This is what the HA add-on watchdog checks; pointing a watchdog at any auth-gated path would read the 401 as "unhealthy" and restart-loop the app
